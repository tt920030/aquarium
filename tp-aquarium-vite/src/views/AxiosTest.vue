<template>
  <div>
    搜尋帳號:<input name="account" type="text" v-model="account" /> 回傳結果:
    <p v-for="data in datas[0]">{{ data.Account }}</p>
  </div>
</template>
...
const getData = function () {
  ...
  axios
    .post(
      `${import.meta.env.VITE_API_URL}AJAX/jQuery/API.php`, //使用get或post等取得路徑資料(php)
      params
    )
