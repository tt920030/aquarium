    ...
    <div class="wrapper">
      ...
      <div class="ticket-year">
        ...
        <div class="ticket-year-right">
          ...
          <RouterLink class="btn" to="/yearpass"
            >前往申請年度通行方案</RouterLink
          >
        </div>
      </div>
      ...
      <p>
        *一般團體不需預約，但是依據人數可接受事前的預約，請於平日9:00~17:00來電窗口諮詢
        電話: 03-1234-567。
      </p>
...
const dayTypes = reactive([
  {
    type: Types[0],
    day: "NT $" + Prices[0],
    night: "NT $" + (Prices[0] - 60),
    for: "18歲(含)~64歲成人使用",
  },
  {
    type: Types[1],
    day: "NT $" + Prices[1],
    night: "NT $" + (Prices[1] - 60),
    for: "12歲(含)~17歲學生使用",
  },
  {
    type: Types[2],
    day: "NT $" + Prices[2],
    night: "NT $" + (Prices[2] - 60),
    for: "6歲(含)~11歲學童使用",
  },
  {
    type: Types[3],
    day: "NT $" + Prices[3],
    night: "NT $" + (Prices[3] - 60),
    for: "持有身心障礙證明者與1位陪同者、孕婦、滿65歲以上長者適用",
  },
]);
const yearTypes = reactive([
  { type: Types[0], price: "NT $" + (Prices[0] * 2 + 40) },
  { type: Types[1], price: "NT $" + (Prices[1] * 2 + 40) },
  { type: Types[2], price: "NT $" + (Prices[2] * 2 + 40) },
  { type: Types[3], price: "NT $" + (Prices[3] * 2 + 40) },
]);
const notices = reactive([
  "購票時請主動出示相關證件供售票處工作人員驗證，且於進場前請攜帶相關證件以備查驗，若無法提供有效證件者需以成人票身份重新購票；符合優惠身份者持票入場後，不可要求部分退款或貼補門票差額。相關證件說明如下：學生票：本人有效學生證正本(若為應屆畢業生則提供當年度正式入學通知單或註冊單及攜帶身分證)；孩童票、博愛票：身心障礙證明、孕婦健康手冊、國民身分證或政府核發附有照片、身分證字號及出生年、月、日等足以證明身分證件",
  "未滿4歲且有家長陪同的幼童可免費入場。",
